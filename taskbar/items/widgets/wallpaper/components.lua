...
sbar.exec(os.getenv("BAR_NAME") .. " --set widgets.background.preview background.image.scale=" .. settings.wallpaper.scale)
...
local function genEntries(dir, name, pos, entryTbl)
    ...
    entryTbl["SELECTED"] = 1
    entryTbl.getSelected = function() if #entryTbl == 0 then return nil end return entryTbl[entryTbl["SELECTED"]] end
